...
date: 2020-09-11T21:13:14-05:00
...
*The build up to the latest release of South Africa's quarterly GDP figures was captivating to say the least, as analysts eagerly waited to affirm their projections on just how bad the second quarter (Q2) would turn out to be. The catastrophic economic performance recorded over Q2 was largely in line with most expectations, but by all means still managed to dominate headlines across the country. Stats SA indicated that an economic contraction of 51% was recorded over Q2 of this year - but does this really mean that half of South Africa's GDP was wiped out in as little as three months? This article offers some insight into why this number might mislead you to think so.* 
...
p1 <- ggplot(dataset, aes(Date, Value, fill = Key)) +
...
To answer this question, one needs to understand how production has fared since the gradual reversal of lockdown measures were initiated. An industrial production index can be a useful tool to analyse this, seeing as this indicator is essentially an informal proxy for monthly GDP. The annual growth rate in the economy's manufacturing sector is shown in Figure \@ref(fig:fig2), which indicates that there has been a partial recovery in production since the historic low recorded in April. A smaller annual contraction recorded over July already represents some improvement compared to how the second quarter started out, though it its important to note that production levels remain at substantially lower levels compared to where they were prior to the pandemic's onset.


```{r fig2, fig.cap='Annual Growth Rates for Monthly Manufacturing Production in South Africa',fig.width=8, tidy=FALSE}
library(rmsfuns)
library(knitr)
pacman::p_load(tidyverse,lubridate,ggthemes,readxl,tsibble,plotly)
...
p1 <- ggplot(dataset, aes(Date, Value)) +
  geom_line(size = 1.2, col = '#4f042e') +
  theme_bw() +
  geom_hline(yintercept = 0, linetype = 'dashed', col = 'red',size = 1) +
  labs(y = 'Annual Percentage Change (%)') +
...
p1 %>% ggplotly() %>% 
  layout(title = list(text = paste0('Industrial Production Index (Manufacturing), South Africa',
                                    '<br>',
                                    '<sup>',
                                    'Manufacturing production has normalised partially since the historic lows it reached in April.',
                                    '</sup>'),x=0.07),margin = m) %>% 
...
Purely on the premise that the economy will be coming off such a low base, it is only practical to expect a partial normalisation in growth over the next few months as lockdown measures are eased further. The unfortunate part about this is that the media's reaction when Q3's figures are released in a few months will most likely be as overblown as what we saw earlier this week. We can expect a sharp rise in annualised growth at the end of Q3 due to this "low base effect" from the second quarter, which will create the false impression that South Africa's economic renaissance has taken full effect.
